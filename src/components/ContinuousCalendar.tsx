...
export const ContinuousCalendar: React.FC<ContinuousCalendarProps> = ({ onClick }) => {
  ...
  // Busca eventos quando o componente monta (sem scroll automático)
  useEffect(() => {
    loadEventos();
  }, []);
...
const loadEventos = async (preserveScroll = false) => {
  ...
  try {
    ...
    // Restaura posição do scroll APÓS recarregar
    if (preserveScroll) {
      setTimeout(() => {
        if (container) {
  ...
  // Função para obter eventos de um dia específico (VERSÃO CORRIGIDA COM UTC)
  ...
  // Função para obter cor do gradiente baseado no índice do evento
...
const handleModalClose = async () => {
  // Salva a posição atual do scroll ANTES de fechar
  const container = document.querySelector('.calendar-container');
  const currentScrollPosition = container?.scrollTop || 0;

  setIsModalOpen(false);
  await loadEventos(); // Recarrega eventos

  // Restaura a posição do scroll DEPOIS de recarregar
  setTimeout(() => {
    if (container) {
      container.scrollTop = currentScrollPosition;
    }
  ...
  return (
    <div className="no-scrollbar calendar-container max-h-full overflow-y-scroll rounded-t-2xl bg-white pb-10 text-slate-800 shadow-xl">
      ...
      <EventDetailsPanel
        ...
        onEventUpdated={() => loadEventos(true)} // ADICIONE true aqui
      />
