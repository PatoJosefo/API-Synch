...
export const EventDetailsPanel: React.FC<EventDetailsPanelProps> = ({
  ...
  const handleEditSuccess = () => {
    setIsEditModalOpen(false);
    onEventUpdated?.(); // Recarrega eventos SEM fechar a sidebar
  };
  ...
  return (
    <>
      {/* Sidebar - só renderiza se isOpen E modal de edição fechado */}
              ...
              {eventos.length === 0 ? (
                ...
                eventos
                  ...
                  .map((evento, idx) => {
                    ...
                    const gradient = getEventGradient(idx);
                          ...
                          {isExpanded && (
                            <div className="px-3 pb-3 space-y-3 pt-3">
                              ...
                              <div>
                                ...
                                <span className={`inline-block mt-1 px-3 py-1 rounded-full text-xs font-medium text-white ${getStatusGradient(evento.statusEvento)}`}>
                              ...
                              {evento.link && evento.link.trim() !== '' ? (
                                <div>
                                  ...
                                  <a href={evento.link} target="_blank" rel="noopener noreferrer" className={`inline-block mt-1 px-3 py-2 rounded-lg text-sm font-medium text-white bg-gradient-to-r ${gradient} hover:opacity-90 transition`}>
                                    🔗 Acessar Reunião
                                  </a>
                                </div>
                              ) : (
                                <div>
                                  <label className="text-xs font-semibold text-gray-600 uppercase">Link da Reunião</label>
                                  <p className="text-sm text-gray-400 mt-1 italic">Nenhum link cadastrado</p>
                                </div>
                              )}
      ...
      {/* Modal de Edição Centralizado - SEMPRE renderiza quando aberto */}
      <EditEventModal
        isOpen={isEditModalOpen}
