<h1 align="center"> Projeto API Synch </h1>
...
- MySQL
### Backend:
Crie uma Database (Base de Dados) em seu MySQL chamada Synch.

```SQL 
CREATE DATABASE Synch;
```
Abra a pasta do projeto nos seu editor de código ou texto preferido, 
- Windows: abra uma instância do CMD,
- Linux: Abra uma instância do terminal Bash
...
```
o Prisma executará as migrations que criarão as tabelas diretamente na base de dados Synch criada anteriormente, se tudo for executado normalmente, ele pedirá que crie um nome para a nova migration, então digite um nome de sua preferência e pressione a tecla Enter de seu teclado.
Depois, digite o seguinte comando e pressione a tecla Enter de seu teclado: 
```bash
npx prisma generate
```
