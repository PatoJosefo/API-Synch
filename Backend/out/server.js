...
app.get('/funcionarios/lista-simples', async (_req, res) => {
    try {
        const funcionarios = await prisma.funcionario.findMany({
            select: {
                id: true,
    ...
    try {
        ...
        const newTemplate = await prisma.formTemplate.create({
            data: {
                ...
                estrutura: templateDto.estrutura,
                criadoPorId,
            },
        });
...
app.put('/form-templates/:id', async (req, res) => {
    try {
        const { id } = req.params;
        if (!id) {
        ...
        if (dadosRecebidos.estrutura !== undefined) {
            dadosParaPrisma.estrutura = dadosRecebidos.estrutura;
        }
...
app.post('/eventos', async (req, res) => {
    ...
    try {
        ...
        if (Array.isArray(convidados) && convidados.length > 0) {
            ...
            const createData = convidados.map((funcId) => {
...
app.get('/eventos/usuario/:funcionarioId', async (req, res) => {
    try {
...
app.put('/eventos/:id/status', async (req, res) => {
    try {
        const eventoId = Number(req.params.id);
...
app.put('/eventos/:eventoId/participantes/:funcionarioId', async (req, res) => {
    try {
        ...
        if (error.code === 'P2025')
            return res.status(404).json({ message: 'Convite ou evento não encontrado.' });
